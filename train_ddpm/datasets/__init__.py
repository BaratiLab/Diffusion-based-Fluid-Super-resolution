...
def data_transform(config, X):
    if config.data.uniform_dequantization: # Default: False
        X = X / 256.0 * 255.0 + torch.rand_like(X) / 256.0
    if config.data.gaussian_dequantization: # Default: False
        X = X + torch.randn_like(X) * 0.01

    if config.data.rescaled: # Default: True
        X = 2 * X - 1.0
    elif config.data.logit_transform: # Default: False
        X = logit_transform(X)

    if hasattr(config, "image_mean"): # Default: False
